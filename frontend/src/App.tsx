...
function App() {
  ...
  useEffect(() => {
    axios.get(`${import.meta.env.VITE_API_URL}/u`, {
  ...
  if (authLoading) {
    ...
    return (
      <div className='max-w-7xl mx-auto'>
        <LoginForm authToggle={authToggle} setAuthToggle={setAuthToggle} />

        <nav className='flex w-full items-center justify-between text-white p-5 bg-[#3f51b5]'>
  ...
  return (
    <div className='max-w-7xl mx-auto'>
      ...
      <nav className='flex w-full justify-between text-white p-5 bg-[#3f51b5]'>
        ...
        <button onClick={() => { setAuthToggle((prev) => !prev) }}>
          Login / Register
        </button>
      </nav>
      <main className='flex flex-col max-w-2xl w-full pt-5 items-center space-y-10 mx-auto'>
